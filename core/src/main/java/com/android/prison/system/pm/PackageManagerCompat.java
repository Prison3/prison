...
import com.android.prison.interfaces.android.content.pm.BRApplicationInfoN;
import com.android.prison.interfaces.android.content.pm.BRPackageParserSigningDetails;
import com.android.prison.interfaces.android.content.pm.BRSigningInfo;
import com.android.prison.interfaces.android.content.res.BRAssetManager;
...
//        ai.uid = baseApplication.uid;
        ...
        if (BuildCompat.isN()) {
            ai.deviceProtectedDataDir = PEnvironment.getDeDataDir(p.packageName, userId).getAbsolutePath();
...
//        }
//        sharedLibraryFileList.add(PEnvironment.JUNIT_JAR.getAbsolutePath());
    ...
    public static Resources getResources(Context context, ApplicationInfo appInfo) {
        PPackageSettings ps = PackageManagerService.get().getBPackageSetting(appInfo.packageName);
        if (ps != null) {
