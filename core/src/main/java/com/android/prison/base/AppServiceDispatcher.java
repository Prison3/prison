...
import com.android.prison.entity.ServiceRecord;
import com.android.prison.manager.PActivityManager;
import com.android.prison.entity.UnbindRecord;
import com.android.prison.proxy.ProxyServiceRecord;
...
//        Log.d(TAG, "onStartCommand: " + component.toString());
        ...
        try {
            ...
            PActivityManager.get().onStartCommand(proxyIntent, stubRecord.mUserId);
            return i;
        } catch (Throwable e) {
            e.printStackTrace();
        }
...
//        Log.d(TAG, "onLowMemory");
    ...
    public boolean onUnbind(Intent proxyIntent) {
        ...
        try {
            UnbindRecord unbindRecord = PActivityManager.get().onServiceUnbind(proxyIntent, PActivityThread.getUserId());
            ...
            if (destroy || record.decreaseConnectionCount(intent)) {
                ...
                if (destroy) {
                    service.onDestroy();
                    PActivityManager.get().onServiceDestroy(proxyIntent, PActivityThread.getUserId());
...
//                Log.d(TAG, "onUnbind：" + stubRecord.mServiceIntent.getComponent().toString());
        ...
        if (record.getService() != null) {
            ...
            try {
                if (destroy) {
                    ...
                    PActivityManager.get().onServiceDestroy(intent, PActivityThread.getUserId());
                    mService.remove(new Intent.FilterComparison(intent));
                }
            } catch (Throwable e) {
                e.printStackTrace();
    ...
    private Service getOrCreateService(ProxyServiceRecord proxyServiceRecord) {
        ...
        Service service = PActivityThread.currentActivityThread().createService(serviceInfo, token);
        if (service == null)
            return null;
