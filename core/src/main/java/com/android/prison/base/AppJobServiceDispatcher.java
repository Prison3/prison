...
//        }
    ...
    JobService getJobService(int jobId) {
        synchronized (mJobRecords) {
            ...
            try {
                JobRecord record = PJobManager.get().queryJobRecord(PActivityThread.getAppProcessName(), jobId);
                record.mJobService = PActivityThread.currentActivityThread().createJobService(record.mServiceInfo);
