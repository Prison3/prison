...
public class GmsManager {
    private static final String TAG = GmsManager.class.getSimpleName();
    ...
    private static InstallResult installPackages(Set<String> list, int userId) {
        ...
        for (String packageName : list) {
            if (PPackageManager.get().isInstalled(packageName, userId)) {
                continue;
            }
            try {
            ...
            InstallResult installResult = PPackageManager.get().installPackageAsUser(packageName, userId);
    ...
    public static boolean isInstalledGoogleService(int userId) {
        return PPackageManager.get().isInstalled(GMS_PKG, userId);
    }
